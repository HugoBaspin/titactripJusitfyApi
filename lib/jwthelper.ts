...
export function encryptToken(object: any) {
  ...
  const encryptedHex: string = `${iv.toString('hex')}:${finalBuffer.toString('hex')}`;
...
export function verifyToken(token: string) {
  ...
  try {
    ...
    const decipher: Decipher = createDecipheriv('aes-128-cbc', Buffer.from(key), iv);
    const decrypted: Buffer = decipher.update(encrypted);
    const clearText: string = Buffer.concat([decrypted, decipher.final()]).toString();
